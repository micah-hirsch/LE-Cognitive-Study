...
wm_pupil <- bam(normed_pupil ~ condition +
                  s(time_norm, by = condition, k = 25) +
                  s(list_sort_c, by = condition, k = 20) +
                  # Random Smooth for listener
                  s(time_norm, listener, bs = 'fs', m = 1),
                discrete = T,
                AR.start = start_event,
...
plot_diff(wm_pupil, view = "list_sort_c",
...
wm_pred <- expand_grid(
  time_norm = seq(from=min(pupil$time_norm), 
              to=max(pupil$time_norm), 
              length.out = 100),
  list_sort_c = seq(from=min(pupil$list_sort_c), 
              to=max(pupil$list_sort_c), 
              length.out = 100),
...
wm_pred <- predict(wm_pupil, newdata = wm_pred, 
                      se.fit = TRUE) |> 
  as_tibble() |>
  cbind(wm_pred)
