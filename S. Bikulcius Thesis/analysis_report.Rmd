...
For our analysis, it is important to consider the correlations between working memory, inhibitory control, cognitive flexibility, processing speed, peak pupil dilation, and perceived listening effort ratings. The figure below shows a correlation matrix. The pearson correlation coefficients are in each block and the color of the block relates to how strong the correlations are. Values closer to 1 or -1 suggest a stronger positive or negative correlation between the two cognitive measures, respectively. The effort measures were averaged per listener across the speakers.

The correlation matrix below shows a moderate positive correlation between cognitive flexibility and processing speed. There are also weak-to-moderate correlations between inhibitory control and cognitive flexibility and processing speed and working memory. There are also weak postive correlations between inhibitory control and cognitive flexibility with peak pupil dilation. Perceived listening effort has a weak negative relationship with working memory and inhibitory control.This means the following:
...
-   Listeners with higher inhibitory control scores tend to have higher peak pupil dilation values
-   Listeners with higher cognitive flexibility scores tend to have higher peak pupil dilation values
-   Listeners with higher working memory scores tend to rate their perceived listening effort lower overall
-   Listeners with higher inhibitory control scores tend to rate perceived listening effort lower overall

All other correlations in the matrix are very weak.
...
overall_corr <- pupil1 |>
  ...
  dplyr::select(subject, trial, speaker, code, peak_pupil, effort_rating, ends_with("_c")) |>
  dplyr::group_by(subject, list_sort_c, flanker_c, card_sort_c, pattern_c) |>
  dplyr::summarize(pupil = mean(peak_pupil),
                   effort = mean(effort_rating)) |>
  ungroup() |>
  dplyr::select(!subject) |>
  dplyr::rename('Working Memory' = list_sort_c,
                'Inhibitory Control' = flanker_c,
                'Cognitive Flexibility' = card_sort_c,
                'Processing Speed' = pattern_c,
                'Peak Pupil Dilation' = pupil,
                'Perceived Listening Effort' = effort) |>
...
#### Correlations for the Control Speaker

We are also looking at these same relationships for each speaker. The correlation matrix for these variables for the control speaker is shown below.

There are some similar correlation patterns between the cognitive measures and both effort measures for the Control speaker compared to the overall matrix above. For instance, there correlations between peak pupil dilation and both cognitive flexibility and inhibitory control were still weak and positive. There was also a weak and negative correlation between inhibitory control and perceived listening effort. However, there are also some noticable differences. To start, there is a weak positive correlation between cognitive flexibility and perceived listening effort ratings. It is also interesting to note there is a stronger relationship between both effort measures (peak pupil dilation and perceived effort) for the control speaker (r = 0.26). This means the following:

-   Listeners who had higher cognitive flexibility scores tend to have greater peak pupil dilation values when listening to the control speaker
-   Listeners who had higher inhibitory control scores tend to have greater peak pupil dilation values when listening to the control speaker
-   Listeners who had higher working memory scores tend to give the control speaker lower perceived listening effort ratings
-   Listeners who had higher inhibitory control scores tend to give the control speaker lower perceived listening effort ratings
-   Listeners who had higher cognitive flexibility scores tend to give the control speaker higher perceived listening effort ratings
-   The weak positive relationship between peak pupil dilation and perceived listening effort ratings means that listeners who had greater peak pupil dilation while listening to the control speaker also tend to give the control speaker higher perceived listening effort ratings. Note that this relationship is stronger compared to when looking at the overall relationship between these measures.
...
control_corr <- pupil1 |>
  dplyr::filter(rep_acc == "accurate") |>
  dplyr::select(subject, speaker, trial, speaker, code, peak_pupil, effort_rating, ends_with("_c")) |>
  dplyr::group_by(subject, speaker, list_sort_c, flanker_c, card_sort_c, pattern_c) |>
  dplyr::summarize(pupil = mean(peak_pupil),
                   effort = mean(effort_rating)) |>
  ungroup() |>
  dplyr::filter(speaker == "Control") |>
  dplyr::select(!c(subject, speaker)) |>
  dplyr::rename('Working Memory' = list_sort_c,
                'Inhibitory Control' = flanker_c,
                'Cognitive Flexibility' = card_sort_c,
                'Processing Speed' = pattern_c,
...
#### Correlations for the ALS Speaker

We also ran a separate correlation matrix so we can examine the relationship between these cognitive measures and effort measures while the participants were listening to the ALS speaker.

Overall, all the correlations between the cognitive measures are the same as the other two matricies above. However, there are some differences noted in the correlations between the cognitive measures and each of the effort measures. To start, the correlation between working memory and peak pupil dilation for the ALS speaker is weak and positive. However, compared to the control speaker, this relationship is stronger for the ALS speaker than the control speaker. The correlations between peak pupil dilation and both inhibitory control and cognitive flexibility also are weaker for the ALS speaker compared to the control speaker. The correlations between perceived listening effort ratings and both inhibitory control and working memory are much weaker for the ALS speaker compared to the control speaker. However, the relationship between perceived listening effort and cognitive flexibility for the ALS speaker is weak-to-moderate and negative. The relationship between the two effort measures is also very small for the ALS speaker (pretty much negligible). This relationship was stronger for the control speaker. Overall, this means:

-   Listeners who had greater working memory scores tend to have greater peak pupil dilation values when listening to the ALS speaker
-   Listeners who had greater cognitive flexibility scores tend to give the ALS speaker lower perceived listening effort ratings
...
corr_plot <- ggpubr::ggarrange(overall_corr, control_corr, ALS_corr, ncol = 3, legend = "bottom", common.legend = T)

ggsave("Figures/Correlation_matricies.png", plot = corr_plot, width = 6, height = 3, units = "in", scale = 2)

rm(ALS_corr, control_corr, overall_corr)
...
ggsave("Figures/peak_pupil_accurate_plot.png", plot = last_plot())
...
ggsave("Figures/peak_pupil_accurate_inaccurate_plot.png", plot = last_plot())
...
ggsave("Figures/cog_peak_pupil_accurate_plot.png", plot = last_plot())
...
ggsave("Figures/cog_peak_pupil_acc_inacc_plot.png", plot = last_plot())
...
m7_standardized <- glmmTMB(scale(peak_pupil) ~ scale(trial_c) + speaker + scale(list_sort_c)*speaker + (1|subject), data = accurate_df)
sjPlot::tab_model(m7_standardized, pred.labels = c("Intercept", "Trial Order", "Speaker [ALS]", "Working Memory", 
                                      "Working Memory * Speaker [ALS]"),
                  dv.labels = "Peak Pupil Dilation",
                  file = "Tables/Peak_pupil_dilation_standardized_model.html")
...
rm(m0, m1, m2, m2b, m3, m3b, m4, m5, m6, m7, m7_standardized, m8, m9, m10, pupil_des)
...
ggsave("Figures/effort_rating_accurate_plot.png", plot = last_plot())
...
ggsave("Figures/effort_rating_acc_inacc_plot.png", plot = last_plot())
...
ggsave("Figures/cog_effort_rating_accurate_plot.png", plot = last_plot())
...
ggsave("Figures/cog_effort_rating_acc_inacc_plot.png", plot = last_plot())
...
Quick Note Here: Working Memory was a significant predictor. However, based on the likelihood ratio test, adding it to the model did not significantly improve model fit, which is why on subsequent steps, we are removing it for now. You'll see that it will eventually get added back in because of the significant interaction between working memory and speaker.
...
m10_standardized <- glmmTMB(scale(effort_rating) ~ scale(trial_c) + speaker + scale(list_sort_c)*speaker + 
                scale(card_sort_c)*speaker + (trial_c|subject) + (trial_c|code), data = accurate_df)
sjPlot::tab_model(m10_standardized, pred.labels = c("Intercept", "Trial Order", "Speaker [ALS]", 
                                      "Working Memory", "Cognitive Flexibility", "Working Memory * Speaker [ALS]",
                                      "Cognitive Flexibility * Speaker [ALS]"), 
                  dv.labels = "Perceived Listening Effort",
                  file = "Tables/perceived_effort_model_standardized.html")
