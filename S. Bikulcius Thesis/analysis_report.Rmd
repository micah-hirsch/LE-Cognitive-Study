...
- The fixed effect of trial order is significant indicating that peak pupil dilation decreased on average by 1.7 units across trials.

```{r, warning = F, message = F}

final <- glmmTMB(peak_pupil ~ trial_c + speaker + scale(list_sort_c)*speaker + (1|subject), data = accurate_df)
sjPlot::tab_model(final, pred.labels = c("Intercept", "Trial Order", "Speaker [ALS]", "Working Memory", 
                                      "Working Memory * Speaker [ALS]"),
                  dv.labels = "Peak Pupil Dilation")
